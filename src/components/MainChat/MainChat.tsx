...
export const MainChat = () => {
  ...
  const roomStatus = useSelector((state: any) => state.generalState.roomStatus);
                      ...
                      <span className={s.activeStatus}>
                        {(onlineUsers[activeChat.toUser.uid] &&
                          onlineUsers[activeChat.toUser.uid].lastSeen >
                            message[1].timestamp) ||
                        onlineUsers[activeChat.toUser.uid].active ? (
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
                            height="16"
                            fill={
                              roomStatus[activeChat.id]?.[
                                activeChat.toUser.uid
                              ] > message[1].timestamp ||
                              onlineUsers[activeChat.toUser.uid].onRoom ===
                                activeChat.id
                                ? "#7c87f2"
                                : "rgb(200,200,200)"
                            }
